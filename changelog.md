...
- Port recipe to new schema system
...
- Add common config to automatically turn off async recipes at instance start to prevent problems with TFC's knapping and alloy recipe types
  - **Note**: This does not edit the file and only applies once, reloading KubeJS' common config (i.e. with `/reload`) will revert the instance back to the setting present in the file
...
- Known (possible) problems/ untested things
  ...
  - The [wiki](https://notenoughmail.github.io/kubejs_tfc/1.20.1/) does not contain some information, but the pages for recipes and data should be accurate
  - Moss growing block types are not fully tested
  - Custom fluid containers filled translation component has been tested
