...
public interface Describe {
    ...
    static <T> int describe(CommandContext<CommandSourceStack> ctx) {
        ...
        if (value == null) {
            return KubeJSTFCCommands.failMsg("%s does not have a value named %s".formatted(dataType.name(), id), ctx);
        }

        final MutableComponent text = Component.empty();
        text.append("\nInfo for %s in %s:\n".formatted(id, dataType.name()));
